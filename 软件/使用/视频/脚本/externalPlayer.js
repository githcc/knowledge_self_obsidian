function init() {
    let buttonElement = document.querySelector("button[is='emby-button'][title='播放']");
	if (buttonElement) {
	    buttonElement.style.display = "none";
        init2();
	}
    buttonElement = document.querySelector("button[is='emby-button'][title='恢复播放']");
    if (buttonElement) {
        buttonElement.style.display = "none";
        init2();
    }
     buttonElement = document.querySelector("button[is='emby-button'][title='预告片']");
	if (buttonElement) {
	    buttonElement.style.display = "none";
	}
}
...
function init2(){
    ...
    let buttonhtml = `<div id="ExternalPlayersBtns" class ="detailButtons flex align-items-flex-start flex-wrap-wrap">
            <button id="embyPot" type="button" class="detailButton  emby-button emby-button-backdropfilter raised-backdropfilter detailButton-primary" title="Potplayer"> <div class="detailButton-content"> <i class="md-icon detailButton-icon button-icon button-icon-left icon-PotPlayer">　</i>  <span class="button-text">播放</span> </div> </button>
            </div>`
...
async function getEmbyMediaInfo() {
    ...
    let streamUrl = itemInfo.MediaSources[0].Path;
    ...
    let domain = streamUrl;
    let subPath = streamUrl;
    let subUrl = streamUrl;
